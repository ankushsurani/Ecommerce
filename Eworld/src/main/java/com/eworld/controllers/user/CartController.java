...
public class CartController {
	...
	public String addToCart(@PathVariable("productId") int productId, Principal principal, HttpSession session, Model model) {
		model.addAttribute("title", "Add CartItem - Eworld");
		
		try {
			
			if(principal == null) {
				session.setAttribute("message", new Msg("Please login if you want to add product in cart", "alert-warning"));
			...
			session.setAttribute("message", new Msg(cart.getProduct().getpName() + " is added to Cart", "alert-success"));
	...
	public String decreaseQuantity(@PathVariable("cartId") int cartId, Principal principal, Model model, HttpSession session) {
	...
	//increse quantity
		...
		public String increseQuantity(@PathVariable("cartId") int cartId, Principal principal, Model model, HttpSession session) {
	...
	//remove cart item
		...
		public String removeCart(@PathVariable("cartId") int cartId, Principal principal, HttpSession session, Model model) {
			...
			try {
				...
				if(cart.getUser().getUserId() == user.getUserId()) {
					...
					session.setAttribute("message", new Msg(cart.getProduct().getpName() + " is removed from Cart", "alert-success"));
				}
